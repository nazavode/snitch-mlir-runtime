...
void snrt_cluster_sw_barrier() {
    ...
    uint32_t barrier = __atomic_add_fetch(&barrier_ptr->barrier, 1, __ATOMIC_RELAXED);
...
static volatile struct snrt_barrier global_barrier __attribute__((section(".dram")));
